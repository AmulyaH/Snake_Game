...
class BaseGameModel:
    ...
    def log_score(self, score, time):
        path = "results/" + self.short_name + ".csv"
        averageTime = score/time
        ...
        with scores_file:
            ...
            writer.writerow([score, time, averageTime])
        self._save_png(path, "runs", "results")

    def stats(self):
        path = "results/" + self.short_name + ".csv"
        if not os.path.exists(path):
            return "(?, ?, ?)"
        scores_file = open(path, 'rU')
        scores = []
        with scores_file:
    ...
    def _save_png(self, input_path, x_label, y_label):
        ...
        plt.savefig("results/" + self.short_name + ".png", bbox_inches="tight")
        plt.close() 
    ...
    def _best_action_for_runs(self, runs):
        ...
        return list(sorted_average_scores_for_actions)[-1]
