...
class Game:
    pygame.init()
    pygame.display.set_caption(Constants.HORCRUX_NAME)
    screen_objects = []
    model = None
    stats = ""
    ...
    def __init__(self, game_model, fps, pixel_size, screen_width, screen_height, navigation_bar_height):
        ...
        while True:
            ...
            if not self.environment.step(ai_action):
                ...
                self.totalTime = round((end - self.start), 2)
                self.start = time.time()
                self.model.log_score(self.environment.reward(),self.totalTime)
                self.stats = self.model.stats()
    ...
    def _draw_screen(self):
        ...
        test = str(self.environment.reward())
        time = "("+str(self.start)+")"
        test += time
